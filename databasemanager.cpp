...
bool DatabaseManager::addSeries(const Series& series) {
    QSqlQuery query;
    query.prepare("INSERT INTO Series (title, genre, starting_date, ending_date, episodes_watched, all_episodes, disc_location, category, grade)"
                  "VALUES (:title, :genre, :starting_date, :ending_date, :episodes_watched, :all_episodes, :disc_location, :category, :grade)");
    ...
    query.bindValue(":disc_location", series.getDiscLocation());
...
QVector<Series> DatabaseManager::getSeries() const {
    ...
    QSqlQuery query("SELECT id, title, genre, starting_date, ending_date, episodes_watched, all_episodes, disc_location, category, grade FROM Series");
    ...
    while(query.next()) {
        Series series(
            ...
            query.value("disc_location").toString(),
...
Series DatabaseManager::getSeriesById(int id) const {
    QSqlQuery query;
    query.prepare("SELECT id, title, genre, starting_date, ending_date, episodes_watched, all_episodes, disc_location, category, grade FROM Series WHERE id = :id");
    ...
    if (query.next()) {
        ...
        QString disc_location = query.value("disc_location").toString();
        QString status = query.value("category").toString();
        int grade = query.value("grade").toInt();

        Series series(id, title, genre, starting_date, ending_date, episodesWatched, allEpisodes, disc_location, status, grade);
        return series;
    } else {
...
bool DatabaseManager::updateSeries(const Series& series) {
    ...
    query.prepare("UPDATE Series SET title = :title, genre = :genre, starting_date = :starting_date, ending_date = :ending_date, "
                  "episodes_watched = :episodes_watched, all_episodes = :all_episodes, disc_location = :disc_location, category = :category, grade = :grade WHERE id = :id");
    query.bindValue(":title", series.getTitle());
    query.bindValue(":genre", series.getGenre());
    query.bindValue(":starting_date", series.getStartingDate());
    query.bindValue(":ending_date", series.getEndingDate());
...
QPair<bool, QVector<Series>> DatabaseManager::getSeriesByTitle(const QString& title) {
    QVector<Series> seriesList;
    QSqlQuery query;
    query.prepare("SELECT id, title, genre, starting_date, ending_date, episodes_watched, all_episodes, disc_location, category, grade "
